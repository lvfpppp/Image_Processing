...
void TFTSPI_Write_Cmd(uint8_t cmd)//明白了
{
  uint8_t i;
  ...
  for(i=0;i<8;i++)
  {
    TFTSPI_SCL=0;//tft18delay_1us(1);	// SCK=0  ILI9163_SCK=0;//感觉就是IIC时序只不过多了DC，CS。IIC只是两根线
...
void TFTSPI_Write_Byte(uint8_t dat)//明白了，跟在TFTSPI_Write_Cmd后做cmd的具体指令内容
{  
  uint8_t i;
...
void TFTSPI_Write_Word(uint16_t dat)//明白了
{  		 
  uint8_t i;
...
/*从第几列开始填充某一行
例：TFTSPI_Show_line(0,5,u16WHITE);  //填充第五行
TFTSPI_Show_Line(2,5,u16WHITE);  //从第二个字符后开始填充第五行  
*/                         //我真的吐了这么多错                                            
void TFTSPI_Show_Line(uint8_t xs,uint8_t ys,uint16_t color)//字符8*16大小,0<=xs<=19,0<=ys<=7
{
  unsigned int i,j;
...
/*
【功能】 填充指定区域、和TFTSPI_Draw_part()一样（改过了）
【参数】 起始点xs坐标，起始点ys坐标，终止点xe坐标，终止点ye坐标，填充色
例：TFTSPI_Fill_Area(0,5,TFT18W,5,u16WHITE);  //填充第五行
*/
...
void TFTSPI_CLS(uint16_t color)
{
  ...
  //TFTSPI_Addr_Rst();用这个有个bug，底部非对称未清理干净
  TFTSPI_Set_Pos(0,0,TFT18W,TFT18H+2);
  for(i=0;i<TFT18W;i++)      //160
  {
    for(j=0;j<TFT18H+2;j++)
    {
      TFTSPI_Write_Word(color);
    }
  }
...
void TFTSPI_Draw_Part(uint8_t xs,uint8_t ys,uint8_t xe,uint8_t ye,uint16_t color_dat)
{
    ...
    /*for(i=0;i<TFT18W;i++)      //160
  {
    for(j=0;j<TFT18H;j++)
    {
      TFTSPI_Write_Word(color_dat);//与下面的没有区别
    }
  }*/
  for(j=0;j<(ye-ys+1);j++)
  {
...
入口参数：圆心横坐标(0-159)，纵坐标(0-127),半径(0-128),显示颜色uint16
出口参数：无
时间：2012-09-09
功能说明：画圆形边框(仅支持屏幕内画圆)
其他说明：0<=x<=159
...
const unsigned char hanzi_Idx[] = //汉字索引//所有汉字太多会出问题
{     
  "十字左右弯单片机智能车器人启动中电压量不足请稍候，！"
};
const unsigned char hanzi16x16[] = {	//阴码，顺向，逐行式
  //列行式 14*16？？？？啥呀
  
  0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0xFF,0xFE,0x01,0x00,//十，0
  0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,
  0x02,0x00,0x01,0x00,0x7F,0xFE,0x40,0x02,0x80,0x04,0x1F,0xE0,0x00,0x40,0x00,0x80,//字，1
  0x01,0x00,0xFF,0xFE,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x05,0x00,0x02,0x00,
  0x02,0x00,0x02,0x00,0x02,0x00,0xFF,0xFE,0x04,0x00,0x04,0x00,0x04,0x00,0x08,0x00,//左，2
  0x0F,0xF8,0x10,0x80,0x10,0x80,0x20,0x80,0x40,0x80,0x80,0x80,0x3F,0xFE,0x00,0x00,
  0x02,0x00,0x02,0x00,0x02,0x00,0xFF,0xFE,0x04,0x00,0x04,0x00,0x08,0x00,0x08,0x00,//右,3
  0x1F,0xF8,0x28,0x08,0x48,0x08,0x88,0x08,0x08,0x08,0x08,0x08,0x0F,0xF8,0x08,0x08,
  0x02,0x00,0x01,0x00,0xFF,0xFE,0x04,0x40,0x14,0x50,0x24,0x48,0x44,0x44,0x00,0x00,//弯，4
  0x3F,0xF0,0x00,0x10,0x3F,0xF0,0x20,0x00,0x3F,0xF8,0x00,0x08,0x00,0x50,0x00,0x20,
  0x21,0x08,0x3F,0xF8,0x01,0x00,0x01,0x00,0xFF,0xFE,0x01,0x00,0x01,0x00,0x01,0x00,//单,2
  0x00,0x40,0x10,0x40,0x10,0x40,0x10,0x40,0x10,0x40,0x1F,0xFC,0x10,0x00,0x10,0x00,
  0x10,0x00,0x1F,0xE0,0x10,0x20,0x10,0x20,0x10,0x20,0x20,0x20,0x20,0x20,0x40,0x20,//片,3
...
const unsigned char hanzi16x12[] = {	
...
void TFTSPI_P16x12Str(unsigned char x,unsigned char y,uint8_t *s_dat,uint16_t word_color,uint16_t back_color)
{
  ...
  while(s_dat[ii] != '\0')
  {
    wm = 0;
    adder = 1;
    while(hanzi_Idx[wm] > 127)//索引汉字所在位置
    {
  ...
  while(s_dat[ii] != '\0')
  {
    ...
    if(adder != 1)// 显示汉字					
    {
      ...
      for(j=0; j <32; j++)
      {
        for(i=0; i<8; i++)
        {
          if((hanzi16x16[adder]) &  (0x80>>i))//先取最高位所以顺向
            TFTSPI_Write_Word(word_color);
          else
            TFTSPI_Write_Word(back_color);
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
uint8_t ColumnarMAX=5;//以下是柱形图三函数
uint8_t Start_X,Start_Y;
uint8_t AreaW,AreaH;
uint8_t Colum_W,Colum_H;
uint16_t BColor,FColor;

//  起始点x,起始点y,频谱宽度w/(ColumnarMAX*2)=柱宽，高度，前景色，背景色、、、最终柱形图宽=柱宽*（DrawSpectrum中画了几个柱形）+（柱宽-1）*（DrawSpectrum中画了几个柱形-1）
...
void DrawingColumnar(uint8_t t,uint8_t duty,uint8_t cl)//duty为数值，只能是0~100的数，最终柱体高度还会取整
{
  unsigned int i,j;  
  ...
  for(i=0;i<Colum_W+2;i++)      //160
  {
    for(j=0;j<Colum_H+2;j++)
    {
      TFTSPI_Write_Word(FColor);//前景色
    }
  }
  
  if(cl)//如果不是最后一个柱体，补充柱体与柱体之间的间隙，即该柱体后下一个柱体前
  {
    //补充空隙
...
{
  //srand((unsigned)time(NULL));没用？？？不懂
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TFTSPI_Show_Pic1(uint8_t xs,uint8_t ys,uint8_t xe,uint8_t ye,uint8_t *ppic)
{
...
void TFTSPI_Show_ALLscreen(uint8_t *pic)//显示整屏幕图片，一般不用
{
  ...
  */	
  ...
  for(i=0;i<=TFT18H;i++)//原来少了个=，图像斜了
  { 
    for(j=0;j<=TFT18W;j++)
    {			
      //TFTSPI_Write_Word(((uint16_t)MYPIC[k])<<8|MYPIC[k+1]);
      TFTSPI_Write_Word(((uint16_t)pic[k])<<8|pic[k+1]);
...
//  {
//    case 1:
//         TFTSPI_Show_Pic2(0,37,160,54,gImage_lo);break;        
//    case 2:
//         TFTSPI_Show_Pic2(0,0,160,92,gImage_shoot);break;  
//    case 3:
//         TFTSPI_Show_Pic2(0,0,160,95,gImage_nap);break; 
//    case 4:
//         TFTSPI_Show_Pic2(0,0,160,118,gImage_stare);break;
//    case 5:
//         TFTSPI_Show_Pic2(0,0,160,95,gImage_lion);break;    
//  }
//}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
